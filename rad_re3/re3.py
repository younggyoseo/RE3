...
class RE3Agent(object):
    ...
    def __init__(
        ...
        # random encoder
        self.random_encoder = hydra.utils.instantiate(random_encoder_cfg).to(
            self.device
        )
        # state entropy
        self.use_state_entropy = use_state_entropy
        self.normalize_state_entropy = normalize_state_entropy
        self.average_state_entropy = average_state_entropy
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            state_entropy = self.random_encoder.compute_state_entropy(
                src_feat, tgt_feat, average_entropy=self.average_state_entropy
            )
            ...
            if self.normalize_state_entropy:
                int_reward = norm_state_entropy
            else:
                int_reward = state_entropy

            if self.use_state_entropy:
                target_Q = (
                    reward
